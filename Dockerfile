...
RUN groupadd -g 44 video && useradd -u 1000 -g video -m appuser
WORKDIR /app
COPY . .
RUN pip3 install --upgrade --ignore-installed pip setuptools wheel && pip3 install -r requirements.txt
RUN chown -R 1000:video /app
USER 1000
ENV QT_OPENGL=desktop
ENV QTWEBENGINE_CHROMIUM_FLAGS="--enable-gpu --ignore-gpu-blacklist --enable-accelerated-video-decode --enable-webgl --enable-logging --v=1 --log-level=0 --disable-gpu-driver-bug-workarounds --use-gl=egl"
